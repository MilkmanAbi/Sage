        ...
        case 'c':
            if (current - start > 1) {
                switch (start[1]) {
                    ...
                    case 'o':
                        if (current - start > 2 && start[2] == 'n') {
                            if (current - start > 3 && start[3] == 't') return check_keyword(2, 6, "ntinue", TOKEN_CONTINUE);
                            if (current - start > 3 && start[3] == 's') return check_keyword(2, 3, "nst", TOKEN_CONST);
                        }
        ...
        case 'i':
            if (current - start > 1) {
                switch (start[1]) {
                    ...
                    case 'm':
                        if (current - start > 2 && start[2] == 'p') {
                            if (current - start > 3 && start[3] == 'o') return check_keyword(2, 4, "port", TOKEN_IMPORT);
                            if (current - start > 3 && start[3] == 'l') return check_keyword(2, 2, "pl", TOKEN_IMPL);
                        }
                        break;
                    case 'n':
...
Token scan_token(void) {
    ...
    switch (c) {
        ...
        case '+': return make_token(match_char('=') ? TOKEN_PLUSASSIGN  : TOKEN_PLUS);
        case '-': return make_token(match_char('>') ? TOKEN_ARROW :
                         match_char('=') ? TOKEN_MINUSASSIGN : TOKEN_MINUS);
        case '*': return make_token(match_char('=') ? TOKEN_STARASSIGN  : TOKEN_STAR);
        case '/': return make_token(match_char('=') ? TOKEN_SLASHASSIGN : TOKEN_SLASH);
        case '%': return make_token(TOKEN_PERCENT);
        case ',': return make_token(TOKEN_COMMA);
        case ':': return make_token(TOKEN_COLON);
        case '.': {
            if (match_char('.')) {
                return make_token(match_char('=') ? TOKEN_DOTDOTEQ : TOKEN_DOTDOT);
            }
            return make_token(TOKEN_DOT);
        }
        case '!': return make_token(match_char('=') ? TOKEN_NEQ : TOKEN_BANG);
        case '?': {
            if (match_char('?')) return make_token(TOKEN_NULLCOAL);
            if (match_char('.')) return make_token(TOKEN_DOTQUESTION);
            return make_token(TOKEN_QUESTION);
        }
        case '=': {
            if (match_char('=')) return make_token(TOKEN_EQ);
            if (match_char('>')) return make_token(TOKEN_FAT_ARROW);
