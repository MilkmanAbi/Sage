...
static Value tcp_connect_native(int argc, Value* args) {
    if (argc >= 1 && IS_STRING(args[0]) && lk_current_sandbox) {
        int port = (argc >= 2 && IS_NUMBER(args[1])) ? (int)AS_NUMBER(args[1]) : 80;
        LKViolation _lkv = lk_check_net_connect(lk_current_sandbox, AS_STRING(args[0]), port);
        if (_lkv != LK_OK) {
            lk_log_violation(lk_current_sandbox, _lkv, AS_STRING(args[0]));
            return val_nil();
        }
    }
...
static Value http_get_native(int argc, Value* args) {
    ...
    // LilyKnight: check net.connect permission
    if (lk_current_sandbox) {
        // Extract hostname from URL (simplified: skip protocol)
        const char* host = url;
        ...
        if (hlen > sizeof(hostname)-1) hlen = sizeof(hostname)-1;
        strncpy(hostname, host, hlen);
        LKViolation _lkv = lk_check_net_connect(lk_current_sandbox, hostname, 443);
        if (_lkv != LK_OK) {
            lk_log_violation(lk_current_sandbox, _lkv, hostname);
            return val_nil();
        }
    }
#ifdef SAGE_HAS_CURL
    ...
    // TODO: implement actual curl call
    curl_easy_cleanup(curl);
#else
    fprintf(stderr, "HTTP not available (build without libcurl)\n");
    return val_nil();
#endif
    return val_string("HTTP GET response placeholder");
}
...
Module* create_legacy_socket_module_DISABLED(ModuleCache* cache) {
