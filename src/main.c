...
static void print_usage(FILE* stream) {
    fprintf(stream,
            "SageTree " SAGE_VERSION_STR "  —  https://github.com/MilkmanAbi/SageTree\n"
            "\n"
            "Usage:\n"
            "  sage                          Start interactive REPL\n"
            "  sage <file.sage>              Run a script\n"
            "  sage -c \"code\"              Evaluate a string\n"
            "  sage --runtime <mode> <file>  Run with explicit runtime\n"
            "\n"
            "Runtimes:\n"
            "  ast     Tree-walk interpreter (default, development)\n"
            "  mossvm  Bytecode VM            (faster, production)\n"
            "\n"
            "Build:\n"
            "  sage --emit-mossvm <file> [-o out.sagec]  Compile to bytecode\n"
            "  sage --run-mossvm  <file.sagec>           Run bytecode\n"
            "  sage --emit-c      <file> [-o out.c]      Emit C source\n"
            "  sage --compile     <file> [-o binary]     Compile to native\n"
            "\n"
            "Library binding:\n"
            "  sage bind <header.h> [--lib libname] [--output file.sage]\n"
            "  sage add  <package>                       Find header + generate bindings\n"
            "\n"
            "Tooling:\n"
            "  sage fmt    <file>            Format source file in-place\n"
            "  sage fmt --check <file>       Check formatting without modifying\n"
            "  sage lint   <file>            Lint source file\n"
            "  sage check  <file>            Type check\n"
            "  sage safety <file>            Safety analysis\n"
            "\n"
            "Memory model:\n"
            "  --gc:tracing   Tracing GC (default)\n"
            "  --gc:arc       Reference counting\n"
            "  --gc:orc       Optimistic reference counting\n"
            "\n"
            "Updates (OIS):\n"
            "  sage --update                 Update to latest version\n"
            "  sage --uninstall              Remove cleanly\n"
            "  sage --ois                    Show install info\n");
}
...
static const char* value_type_name(Value v) {
    switch (v.type) {
        ...
        case VAL_VM_PROGRAM: return "mossvm-program";
...
static void repl_print_help(void) {
    printf("SageTree REPL — commands start with ':'\n");
    printf("\n");
    printf("  :help                Show this help\n");
    printf("  :quit  / :exit       Exit  (Ctrl-D also works)\n");
    printf("  :reset               Reset session and module cache\n");
    printf("  :clear               Clear the screen\n");
    printf("  :history [n]         Show last n entries (default 20)\n");
    printf("  :save <file>         Save session to a .sage file\n");
    printf("\n");
    printf("  :vars [prefix]       List defined names\n");
    printf("  :type <expr>         Evaluate and show type\n");
    printf("  :modules             List loaded modules\n");
    printf("  :env                 Show scope chain\n");
    printf("\n");
    printf("  :time <expr>         Time an expression\n");
    printf("  :bench <n> <expr>    Benchmark: run n times, print stats\n");
    printf("  :gc                  Run GC and print stats\n");
    printf("\n");
    printf("  :runtime             Show current runtime (ast / mossvm)\n");
    printf("  :runtime ast         Switch to tree-walk interpreter\n");
    printf("  :runtime mossvm      Switch to bytecode VM\n");
    printf("  :pwd / :cd <dir>     Working directory\n");
    printf("\n");
    printf("Multi-line: blocks ending in ':' continue until a blank line.\n");
}
...
static void repl_print_ast_expr(Expr* expr, int depth) {
    ...
    switch (expr->type) {
        case EXPR_NUMBER: printf("(float %g)", expr->as.number.value); break;
        case EXPR_INT: printf("(int %" PRId64 ")", expr->as.int_val.value); break;
...
static void run_repl(SageRuntimeMode runtime_mode) {
    printf("SageTree " SAGE_VERSION_STR "\n");
    printf("Type help for help, quit to exit.\n");

    // P2: Force AST runtime in REPL to avoid MossVM first-statement bugs
    // and to get type checking, struct copy, etc. working properly
    runtime_mode = SAGE_RUNTIME_AST;
    ...
    while (1) {
        ...
        // Handle REPL commands — both plain and :prefix
        ...
        if (strcmp(line, "help") == 0 || strcmp(line, ":help") == 0) {
            printf("SageTree REPL commands:\n");
            printf("  help              Show this help\n");
            printf("  quit / exit       Exit the REPL\n");
            printf("  vars [pattern]    List defined variables\n");
            printf("  clear             Clear screen\n");
            printf("  history [n]       Show command history\n");
            printf("  type <expr>       Show type of expression\n");
            printf("  time <expr>       Time an expression\n");
            printf("  save <file>       Save session to file\n");
            printf("  load <file>       Load and run a file\n");
            printf("\nAll commands also work with : prefix (:help, :quit, etc.)\n");
            free(line);
            continue;
        }

        if (line[0] == ':' || strncmp(line, "vars", 4) == 0) {
            const char* arg = NULL;

            if (command_matches(line, "vars", &arg) || command_matches(line, ":vars", &arg)) {
                repl_list_bindings(env, (*arg != '\0') ? arg : NULL);
                free(line);
                continue;
            }

            if (command_matches(line, "pwd", &arg) || command_matches(line, ":pwd", NULL)) {
                char cwd[4096];
            ...
            if (command_matches(line, "cd", &arg) || command_matches(line, ":cd", &arg)) {
                if (*arg == '\0') {
                    printf("Usage: :cd <dir>\n");
                } else if (chdir(arg) != 0) {
                    perror("chdir");
            ...
            if (command_matches(line, "gc", NULL) || command_matches(line, ":gc", NULL)) {
            ...
            if (command_matches(line, "load", &arg) || command_matches(line, ":load", &arg)) {
                if (*arg == '\0') {
            ...
            if (command_matches(line, "type", &arg) || command_matches(line, ":type", &arg)) {
                if (*arg == '\0') {
            ...
            // :clear — clear screen
            if (command_matches(line, "clear", NULL) || command_matches(line, ":clear", NULL)) {
                printf("\033[2J\033[H");
                fflush(stdout);
                free(line);
                continue;
            ...
            if (command_matches(line, "history", &arg) || command_matches(line, ":history", &arg)) {
                int n = 20;
                if (*arg != '\0') n = atoi(arg);
                if (n <= 0) n = 20;
            ...
            if (command_matches(line, "save", &arg) || command_matches(line, ":save", &arg)) {
                if (*arg == '\0') {
            ...
            if (command_matches(line, "time", &arg) || command_matches(line, ":time", &arg)) {
                if (*arg == '\0') {
            ...
            if (command_matches(line, "bench", &arg) || command_matches(line, ":bench", &arg)) {
                if (*arg == '\0') {
            ...
            if (command_matches(line, "runtime", &arg) || command_matches(line, ":runtime", &arg)) {
                if (*arg == '\0') {
                    ...
                    if (sage_runtime_parse_mode(arg, &new_mode)) {
                        ...
                        printf("Unknown runtime mode: %s (use: ast, mossvm)\n", arg);
                    }
                }
                free(line);
                continue;
...
int main(int argc, const char* argv[]) {
    ...
    // P9: Initialize Firefly error diagnosis
    firefly_init();
    ...
    if (cmd_argc >= 3 && strcmp(cmd_argv[1], "--runtime") == 0) {
        if (!sage_runtime_parse_mode(cmd_argv[2], &runtime_mode)) {
            fprintf(stderr, "Unknown runtime mode: %s (expected: ast, mossvm)\n", cmd_argv[2]);
            print_usage(stderr);
            CLEANUP_AND_EXIT(64);
        }
        cmd_argv += 2;
    ...
    // --firefly=full|minimal|off — Firefly verbosity
    for (int fi = 1; fi < cmd_argc; fi++) {
        if (strncmp(cmd_argv[fi], "--firefly=", 10) == 0) {
            const char* mode = cmd_argv[fi] + 10;
            if (strcmp(mode, "full") == 0) firefly_set_verbosity(FIREFLY_FULL);
            else if (strcmp(mode, "minimal") == 0) firefly_set_verbosity(FIREFLY_MINIMAL);
            else if (strcmp(mode, "off") == 0) firefly_set_verbosity(FIREFLY_OFF);
            else fprintf(stderr, "Unknown Firefly mode: %s (expected: full, minimal, off)\n", mode);
            // Shift remaining args
            for (int j = fi; j < cmd_argc - 1; j++) cmd_argv[j] = cmd_argv[j+1];
            cmd_argc--;
            fi--;
        }
    }
    ...
    if (cmd_argc == 1) {
        ...
        // Explicit REPL flag
        run_repl(runtime_mode);
    } else if (cmd_argc == 2 && strcmp(cmd_argv[1], "--help") == 0 || strcmp(cmd_argv[1], "-h") == 0) {
        print_usage(stdout);
    } else if (strcmp(cmd_argv[1], "--version") == 0 || strcmp(cmd_argv[1], "-V") == 0) {
        printf("SageTree %s\n", SAGE_VERSION_STR);
    } else if (strcmp(cmd_argv[1], "--info") == 0) {
        printf("SageTree %s\n", SAGE_VERSION_STR);
        printf("\n");
        printf("  MossVM        Bytecode virtual machine + AST interpreter\n");
        printf("  SageGC        Tri-color mark-sweep garbage collector\n");
        printf("  LilyBox       Sandboxed execution environment\n");
        printf("  LilyKnight    Permission enforcement (capability-based)\n");
        printf("  FrogPond      Sandbox user API (import sandbox)\n");
        printf("  Firefly       Error diagnosis subsystem (in development)\n");
        printf("\n");
        printf("  Types:        int (i64), float (f64), str, bool, Array, Dict, Tuple\n");
        printf("  Memory:       GC (default), @manual, @safe, hybrid\n");
        printf("  FFI:          C (libffi + sage bind), Python (CPython embedded)\n");
#ifdef SAGE_EXPERIMENTAL_JIT
        printf("  JIT:          enabled (experimental, x86-64 method JIT)\n");
#else
        printf("  JIT:          disabled (experimental, rebuild with -DSAGE_EXPERIMENTAL_JIT)\n");
#endif
#ifndef SAGE_NO_PYTHON
        printf("  Python:       enabled\n");
#else
        printf("  Python:       disabled (python3-dev not found)\n");
#endif
        printf("  Platform:     %s\n",
#ifdef __linux__
            "Linux"
        ...
        free(derived_output);
               ...
               (strcmp(cmd_argv[1], "--emit-mossvm") == 0 || strcmp(cmd_argv[1], "--emit-bytecode") == 0)) {
        const char* explicit_output = NULL;
        const char* ignored_cc = NULL;
        const char* ignored_target = NULL;
        int opt_level = 0, debug_info = 0;
        ...
        const char* output_path = explicit_output;
        if (output_path == NULL) {
            derived_output = derive_output_path(cmd_argv[2], ".mvm", 1);
            output_path = derived_output;
        }
        ...
        free(derived_output);
               ...
               (strcmp(cmd_argv[1], "--run-mossvm") == 0 || strcmp(cmd_argv[1], "--run-bytecode") == 0)) {
    ...
    } else if (cmd_argc >= 2 && strcmp(cmd_argv[1], "bind") == 0) {
        // Phase 4: Library attachment — generate Sage bindings from C header
        // Usage: sage bind <header.h> [--lib libname] [--output file.sage] [-- <clang-flags>]
        if (cmd_argc < 3) {
            fprintf(stderr, "Usage: sage bind <header.h> [--lib libname] [--output file.sage]\n");
            CLEANUP_AND_EXIT(64);
        }
        // Find sage-bind binary next to the sage binary
        char bind_path[4096];
        {
            ...
            if (len > 0) {
                ...
                if (slash) {
                    *slash = '\0';
                    snprintf(bind_path, sizeof(bind_path), "%s/sage-bind", self_path);
                } else {
                    strcpy(bind_path, "sage-bind");
                }
            } else {
                strcpy(bind_path, "sage-bind");
            }
        }
        // Exec sage-bind with remaining args
        execv(bind_path, (char* const*)(cmd_argv + 1));
        // Fallback: try PATH
        execvp("sage-bind", (char* const*)(cmd_argv + 1));
        perror("sage bind: could not find sage-bind binary");
        CLEANUP_AND_EXIT(1);

    } else if (cmd_argc >= 2 && strcmp(cmd_argv[1], "add") == 0) {
        // Phase 4: sage add <package> — install system lib + generate bindings
        // This is a thin wrapper that calls the OIS installer + sage bind
        if (cmd_argc < 3) {
            fprintf(stderr, "Usage: sage add <library-name>\n");
            fprintf(stderr, "Example: sage add sqlite3\n");
            CLEANUP_AND_EXIT(64);
        }
        const char* pkg = cmd_argv[2];
        // Try to find the system header
        char header_path[512];
        char so_name[256];
        snprintf(so_name, sizeof(so_name), "lib%s", pkg);
        // Common header locations
        const char* search_dirs[] = {
            "/usr/include", "/usr/local/include",
            "/usr/include/x86_64-linux-gnu", NULL
        };
        int found = 0;
        for (int i = 0; search_dirs[i] && !found; i++) {
            snprintf(header_path, sizeof(header_path), "%s/%s.h", search_dirs[i], pkg);
            FILE* f = fopen(header_path, "r");
            if (f) { fclose(f); found = 1; }
        }
        if (!found) {
            fprintf(stderr, "sage add: header <%s.h> not found in system include paths\n", pkg);
            fprintf(stderr, "  Install the dev package first: sudo apt install lib%s-dev\n", pkg);
            CLEANUP_AND_EXIT(1);
        }
        // Run sage-bind
        printf("sage add: generating bindings from %s...\n", header_path);
        char out_file[512];
        snprintf(out_file, sizeof(out_file), "%s.sage", pkg);
        char bind_path[4096];
        {
            char self_path[4096] = {0};
            ssize_t len = readlink("/proc/self/exe", self_path, sizeof(self_path)-1);
            if (len > 0) {
                self_path[len] = '\0';
                char* slash = strrchr(self_path, '/');
                if (slash) { *slash = '\0'; snprintf(bind_path, sizeof(bind_path), "%s/sage-bind", self_path); }
                else strcpy(bind_path, "sage-bind");
            } else strcpy(bind_path, "sage-bind");
        }
        char* bind_argv[] = { "sage-bind", header_path, "--lib", so_name, "--output", out_file, NULL };
        execv(bind_path, bind_argv);
        execvp("sage-bind", bind_argv);
        perror("sage add: could not find sage-bind binary");
        CLEANUP_AND_EXIT(1);
...
#ifndef SAGE_EXPERIMENTAL_JIT
        fprintf(stderr, "error: JIT is experimental and disabled in this build.\n");
        fprintf(stderr, "       Rebuild with: make CFLAGS+=-DSAGE_EXPERIMENTAL_JIT\n");
        fprintf(stderr, "\n");
        fprintf(stderr, "       The JIT compiler is experimental and being evaluated.\n");
        fprintf(stderr, "       Future plan: LuaJIT-based tracing JIT customised for Sage.\n");
        CLEANUP_AND_EXIT(1);
#else
        ...
        fprintf(stderr, "[experimental] JIT: Enabled (threshold=%d calls, pool=%zuKB)\n",
        ...
        jit_shutdown(&jit);
        free(source);
#endif // SAGE_EXPERIMENTAL_JIT
    } else if (cmd_argc >= 4 && strcmp(cmd_argv[1], "--aot") == 0 && strcmp(cmd_argv[2], "--jit") == 0) {
#ifndef SAGE_EXPERIMENTAL_JIT
        fprintf(stderr, "error: AOT+JIT is experimental. Rebuild with: make CFLAGS+=-DSAGE_EXPERIMENTAL_JIT\n");
        CLEANUP_AND_EXIT(1);
#else
        ...
        AotCompiler* _aot_ptr = (AotCompiler*)calloc(1, sizeof(AotCompiler));
        if (!_aot_ptr) { fprintf(stderr, "AOT: out of memory\n"); CLEANUP_AND_EXIT(1); }
        aot_init(_aot_ptr, 2);
        _aot_ptr->emit_guards = 1; // Enable type guards from JIT data
        ...
        for (int i = 0; i < jit.profile_count; i++) {
            ...
            if (p && p->call_count > 0 && p->return_type != JIT_TYPE_UNKNOWN) {
                ...
                aot_set_var_type(_aot_ptr, name, p->return_type);
            }
        }

        char* c_code = aot_compile_program(_aot_ptr, ast);
        ...
        if (out_path) {
            ...
            if (aot_compile_to_binary(_aot_ptr, c_path, out_path)) {
                int profiled = 0;
                for (int i = 0; i < jit.profile_count; i++)
                    if (jit.profiles[i] && jit.profiles[i]->call_count > 0) profiled++;
        ...
        AotCompiler* _aot_ptr2 = (AotCompiler*)calloc(1, sizeof(AotCompiler));
        if (!_aot_ptr2) { fprintf(stderr, "AOT: out of memory\n"); free(source); CLEANUP_AND_EXIT(1); }
        aot_init(_aot_ptr2, 2); // -O2 default
        char* c_code = aot_compile_program(_aot_ptr2, ast);
        ...
        if (out_path) {
            ...
            FILE* f = fopen(c_path, "w");
            if (f) { fputs(c_code, f); fclose(f); }
            if (aot_compile_to_binary(_aot_ptr2, c_path, out_path)) {
        ...
        module_add_source_dir(cmd_argv[1]);
        char* source = main_read_file(cmd_argv[1]);
        
        // P14: Run Firefly warning pass before execution
        firefly_set_file(cmd_argv[1]);
        {
            const char* warn_src = source;
            init_lexer(warn_src, cmd_argv[1]);
            parser_init();
            Stmt* full_ast = parse_program(warn_src, cmd_argv[1]);
            if (full_ast) {
                firefly_warn_pass(full_ast, cmd_argv[1]);
            }
        }
