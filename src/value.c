...
const Value sage_nil = {VAL_NIL, {.integer = 0}};
...
Value string_join(Value* arr, const char* separator) {
    ...
    // First pass: convert all elements to strings, measure total length
    char** strs = SAGE_ALLOC(sizeof(char*) * a->count);
    size_t* lens = SAGE_ALLOC(sizeof(size_t) * a->count);
    size_t total_len = 0;

    for (int i = 0; i < a->count; i++) {
        Value elem = a->elements[i];
        if (IS_STRING(elem)) {
            strs[i] = AS_STRING(elem);
            lens[i] = strlen(strs[i]);
        } else if (elem.type == VAL_INT) {
            char buf[32];
            snprintf(buf, sizeof(buf), "%" PRId64, AS_INT(elem));
            strs[i] = SAGE_ALLOC(strlen(buf) + 1);
            strcpy(strs[i], buf);
            lens[i] = strlen(strs[i]);
        } else if (elem.type == VAL_NUMBER) {
            char buf[32];
            double n = AS_NUMBER(elem);
            if (n == (long long)n && n >= -9007199254740992.0 && n <= 9007199254740992.0)
                snprintf(buf, sizeof(buf), "%lld.0", (long long)n);
            else
            ...
            strs[i] = AS_BOOL(elem) ? "true" : "false";
            lens[i] = strlen(strs[i]);
    ...
    *wp = '\0';
    ...
    // Free allocated string buffers (not the ones that point to existing strings)
    for (int i = 0; i < a->count; i++) {
        Value elem = a->elements[i];
        if (elem.type == VAL_INT || elem.type == VAL_NUMBER) {
            free(strs[i]);
        }
    }
    free(strs);
...
ClassValue* class_create(const char* name, int name_len, ClassValue* parent) {
    ...
    class_val->is_struct = 0;  // P2: default to reference type
    gc_unpin();
    return class_val;
}
...
// P2: Deep copy an instance (for struct value semantics)
Value instance_copy(Value original) {
    if (!IS_INSTANCE(original)) return original;
    InstanceValue* orig = original.as.instance;
    gc_pin();
    InstanceValue* copy = instance_create(orig->class_def);
    // Copy all fields
    if (orig->fields) {
        Value orig_dict;
        orig_dict.type = VAL_DICT;
        orig_dict.as.dict = orig->fields;
        for (int i = 0; i < orig->fields->capacity; i++) {
            if (orig->fields->entries[i].key != NULL) {
                Value copy_dict;
                copy_dict.type = VAL_DICT;
                copy_dict.as.dict = copy->fields;
                dict_set(&copy_dict, orig->fields->entries[i].key, orig->fields->entries[i].value);
                copy->fields = copy_dict.as.dict;
            }
        }
    }
    gc_unpin();
...
void print_value(Value v) {
    ...
    switch (v.type) {
        case VAL_INT: {
            printf("%" PRId64, AS_INT(v));
            break;
        }
        case VAL_NUMBER: {
            double n = AS_NUMBER(v);
            // Always show decimal point for floats so 42.0 != 42
            if (n == (long long)n && n >= -9007199254740992.0 && n <= 9007199254740992.0) {
                printf("%lld.0", (long long)n);
            } else {
                printf("%g", n);
            }
            break;
        ...
        case VAL_DICT: {
            // Pretty-print Option and Result tagged dicts
            DictValue* d = v.as.dict;
            Value type_tag = dict_get(&v, "__type");
            if (IS_STRING(type_tag)) {
                const char* tag = AS_STRING(type_tag);
                Value inner = dict_get(&v, "__val");
                if (strcmp(tag, "option.some") == 0) {
                    printf("Some(");
                    print_value(inner);
                    printf(")");
                    break;
                } else if (strcmp(tag, "result.ok") == 0) {
                    printf("Ok(");
                    print_value(inner);
                    printf(")");
                    break;
                } else if (strcmp(tag, "result.err") == 0) {
                    printf("Err(");
                    print_value(inner);
                    printf(")");
                    break;
                } else if (strcmp(tag, "range") == 0) {
                    Value lo = dict_get(&v, "low");
                    Value hi = dict_get(&v, "high");
                    Value inc = dict_get(&v, "inclusive");
                    print_value(lo);
                    printf(IS_BOOL(inc) && AS_BOOL(inc) ? "..=" : "..");
                    print_value(hi);
                    break;
                }
            }
            ...
            for (int i = 0; i < d->capacity; i++) {
                if (d->entries[i].key != NULL) {
                    // Skip internal __type / __val fields
                    if (strncmp(d->entries[i].key, "__", 2) == 0) continue;
                    if (printed > 0) printf(", ");
...
int values_equal(Value a, Value b) {
    // P1: numeric cross-comparison (42 == 42.0 is true)
    if (IS_NUMERIC(a) && IS_NUMERIC(b)) {
        // If both are ints, compare as ints (exact)
    ...
    switch (a.type) {
        case VAL_INT:    return AS_INT(a) == AS_INT(b);  // already handled above, but safe
