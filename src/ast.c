...
// P1: Integer literal expression
Expr* new_int_expr(int64_t value) {
    Expr* e = SAGE_ALLOC(sizeof(Expr));
    e->type = EXPR_INT;
    e->as.int_val.value = value;
    return e;
}
...
Stmt* new_let_stmt(Token name, Expr* initializer) {
    ...
    s->as.let.is_mutable = 0;  // P4: default immutable (caller sets for var)
    s->next = NULL;
    s->pragmas = NULL;
    return s;
}
...
Stmt* new_annotated_block_stmt(Stmt* statements, BlockAnnotation annotation, const char* arg) {
    Stmt* s = SAGE_ALLOC(sizeof(Stmt));
    s->type = STMT_ANNOTATED_BLOCK;
    s->as.annotated_block.statements = statements;
    s->as.annotated_block.annotation = annotation;
    if (arg) {
        strncpy(s->as.annotated_block.annotation_arg, arg, 63);
        s->as.annotated_block.annotation_arg[63] = '\0';
    } else {
        s->as.annotated_block.annotation_arg[0] = '\0';
    }
    s->next = NULL;
    s->pragmas = NULL;
    return s;
...
void free_expr(Expr* expr) {
    ...
    switch (expr->type) {
        ...
        case EXPR_COMPTIME:
            ...
            break;
        case EXPR_NUMBER: case EXPR_INT:
        case EXPR_BOOL:
        case EXPR_NIL:
        case EXPR_VARIABLE:
            break;
...
// Phase 2: Option/Result expression constructors
Expr* new_force_unwrap_expr(Expr* operand) {
    Expr* e = SAGE_ALLOC(sizeof(Expr));
    e->type = EXPR_FORCE_UNWRAP;
    e->as.unwrap.operand = operand;
    return e;
}

Expr* new_propagate_expr(Expr* operand) {
    Expr* e = SAGE_ALLOC(sizeof(Expr));
    e->type = EXPR_PROPAGATE;
    e->as.unwrap.operand = operand;
    return e;
}
...
Expr* new_nullcoal_expr(Expr* left, Expr* right) {
    Expr* e = SAGE_ALLOC(sizeof(Expr));
    e->type = EXPR_NULLCOAL;
    e->as.nullcoal.left  = left;
    e->as.nullcoal.right = right;
    return e;
}

Expr* new_optchain_expr(Expr* object, Token member) {
    Expr* e = SAGE_ALLOC(sizeof(Expr));
    e->type = EXPR_OPTCHAIN;
    e->as.optchain.object = object;
    e->as.optchain.member = member;
    return e;
}
