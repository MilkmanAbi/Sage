...
const char* sage_runtime_mode_name(SageRuntimeMode mode) {
    switch (mode) {
        ...
        case SAGE_RUNTIME_MOSSVM: return "mossvm";
...
int sage_runtime_parse_mode(const char* text, SageRuntimeMode* mode_out) {
    ...
    if (strcmp(text, "mossvm") == 0 || strcmp(text, "moss") == 0 || strcmp(text, "bytecode") == 0) {
        *mode_out = SAGE_RUNTIME_MOSSVM;
        return 1;
    }
...
ExecResult sage_execute_stmt(Stmt* stmt, Env* env, SageRuntimeMode mode) {
    ...
    // P1: For MossVM/AOT paths — walk the statement list and inject type checks
    // Type annotation enforcement happens at the interpreter level for AST mode,
    // but MossVM compiles directly to bytecode. We handle this by falling back
    // to AST interpret for annotated let statements, or by pre-evaluating.
    // For now: if the top-level statement is a LET with type_ann and we're in
    // MossVM mode, we force AST mode for that statement so type checks run.
    if (mode == SAGE_RUNTIME_AUTO || mode == SAGE_RUNTIME_MOSSVM) {
        // Recursively scan for features that need AST fallback
        int needs_ast = 0;
        Stmt* stack[64];
        int sp = 0;
        stack[sp++] = stmt;
        while (sp > 0 && !needs_ast) {
            Stmt* cur = stack[--sp];
            while (cur && !needs_ast) {
                // P2: struct defs need AST for copy-on-assignment
                if (cur->type == STMT_STRUCT) needs_ast = 1;
                if (cur->type == STMT_IMPL) needs_ast = 1;
                if (cur->type == STMT_ENUM) needs_ast = 1;
                if (cur->type == STMT_BLOCK && sp < 63) {
                    stack[sp++] = cur->as.block.statements;
                }
                cur = cur->next;
            }
        }
        if (needs_ast) return interpret(stmt, env);
    }

    if (mode == SAGE_RUNTIME_AUTO) {
        // Auto mode: use MossVM bytecode by default.
        // JIT is experimental and must be opted into explicitly.
    }
