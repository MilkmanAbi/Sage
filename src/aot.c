...
void aot_infer_types(AotCompiler* aot, Stmt* program) {
    for (Stmt* s = program; s; s = s->next) {
        ...
        if (s->type == STMT_PROC || s->type == STMT_ASYNC_PROC) {
            ...
            aot_register_proc(aot, cn); free(cn);
        }
    ...
    switch (expr->type) {
        ...
        case EXPR_BINARY: {
            ...
            if (op==TOKEN_AND) {
                char* lv=aot_expr(aot,expr->as.binary.left,JIT_TYPE_UNKNOWN);
                char* rv=aot_expr(aot,expr->as.binary.right,JIT_TYPE_UNKNOWN);
                char* out=malloc(strlen(lv)+strlen(rv)*2+32);
            ...
            if (op==TOKEN_OR) {
                char* lv=aot_expr(aot,expr->as.binary.left,JIT_TYPE_UNKNOWN);
                char* rv=aot_expr(aot,expr->as.binary.right,JIT_TYPE_UNKNOWN);
                char* out=malloc(strlen(lv)*2+strlen(rv)+32);
        ...
        case EXPR_FORCE_UNWRAP: {
            ...
            char* out=malloc(strlen(inner)+80);
            sprintf(out,"({SageValue _fu=(%s);if(SAGE_IS_NIL(_fu))sage_rt_fatal(\"force-unwrap on nil\");_fu;})",inner);
            free(inner); return out;
        }
        case EXPR_PROPAGATE: {
            char* inner=aot_expr(aot,expr->as.unwrap.operand,JIT_TYPE_UNKNOWN);
            char* out=malloc(strlen(inner)+64);
            sprintf(out,"({SageValue _pp=(%s);if(SAGE_IS_NIL(_pp))return sage_rt_nil();_pp;})",inner);
            free(inner); return out;
        }
        ...
        case EXPR_ARRAY: {
            ...
            for(int i=0;i<n;i++){char*e=aot_expr(aot,expr->as.array.elements[i],JIT_TYPE_UNKNOWN);sz+=strlen(e)+64;free(e);}
            char* out=malloc(sz);
            int pos=sprintf(out,"({SageValue %s=sage_rt_array_new();",tmp);
            for(int i=0;i<n;i++){char*e=aot_expr(aot,expr->as.array.elements[i],JIT_TYPE_UNKNOWN);pos+=sprintf(out+pos,"sage_rt_array_push(%s,%s);",tmp,e);free(e);}
        ...
        case EXPR_INDEX: {
            ...
            char* obj=aot_expr(aot,expr->as.index.array,JIT_TYPE_UNKNOWN);
            char* idx=aot_expr(aot,expr->as.index.index,JIT_TYPE_UNKNOWN);
            char* out=malloc(strlen(obj)+strlen(idx)+64);
            if (objt==JIT_TYPE_DICT || idxt==JIT_TYPE_STRING)
                sprintf(out,"sage_rt_dict_get(%s,%s)",obj,idx);
            else
                sprintf(out,"sage_rt_array_get(%s,%s)",obj,idx);
            free(obj);free(idx); return out;
        }
        case EXPR_INDEX_SET: {
            char* obj=aot_expr(aot,expr->as.index_set.array,JIT_TYPE_UNKNOWN);
            char* idx=aot_expr(aot,expr->as.index_set.index,JIT_TYPE_UNKNOWN);
            char* val=aot_expr(aot,expr->as.index_set.value,JIT_TYPE_UNKNOWN);
            char* out=malloc(strlen(obj)+strlen(idx)+strlen(val)+48);
        ...
        case EXPR_CALL: {
            ...
            if (expr->as.call.callee && expr->as.call.callee->type==EXPR_VARIABLE) {
                ...
                // join() builtin
                // Interpreter-only FFI builtins — return nil in AOT mode
                if (BM("struct_def")||BM("struct_new")||BM("struct_get")||
                    BM("struct_set")||BM("struct_size")||BM("mem_free")||
                    BM("mem_alloc")||BM("mem_read")||BM("mem_write")) {
                    char*o=malloc(64); snprintf(o,64,"sage_rt_nil()/*%.*s:interpreter-only*/",rawlen,(const char*)raw);
                    return o;
                }
                ...
                if (BM("dict_has")&&argc==2){char*a=aot_expr(aot,expr->as.call.args[0],JIT_TYPE_UNKNOWN);char*b=aot_expr(aot,expr->as.call.args[1],JIT_TYPE_UNKNOWN);size_t sz=strlen(a)+strlen(b)+64;char*o=malloc(sz);snprintf(o,sz,"sage_rt_bool(sage_rt_dict_has(%s,%s))",a,b);free(a);free(b);return o;}
                ...
                if (BM("push")&&argc==2){char*a=aot_expr(aot,expr->as.call.args[0],JIT_TYPE_UNKNOWN);char*b=aot_expr(aot,expr->as.call.args[1],JIT_TYPE_UNKNOWN);size_t sz=strlen(a)+strlen(b)+64;char*o=malloc(sz);snprintf(o,sz,"({sage_rt_array_push(%s,%s);sage_rt_nil();})",a,b);free(a);free(b);return o;}
                ...
                if (BM("abs")&&argc==1){char*a=aot_expr(aot,expr->as.call.args[0],JIT_TYPE_UNKNOWN);size_t sz=strlen(a)+64;char*o=malloc(sz);snprintf(o,sz,"(SAGE_IS_INT(%s)?sage_rt_int(llabs(%s.as.integer)):sage_rt_float(fabs(%s.as.number)))",a,a,a);free(a);return o;}
                ...
                for(int i=0;i<argc;i++){
                    ...
                    // If param is typed but call site has SageValue arg, box it
            ...
            if (expr->as.call.callee && expr->as.call.callee->type == EXPR_GET) {
                ...
                if (!strcmp(_mn,"contains_key")&&argc==1){ char*a=aot_expr(aot,expr->as.call.args[0],JIT_TYPE_UNKNOWN);size_t _cs=strlen(_mobj)+strlen(a)+256;char*o=malloc(_cs);snprintf(o,_cs,"sage_rt_bool(sage_rt_dict_has(%s,%s))",_mobj,a);free(_mobj);free(_mn);free(a);return o;}
                ...
                if (!strcmp(_mn,"push")&&argc==1){char*a=aot_expr(aot,expr->as.call.args[0],JIT_TYPE_UNKNOWN);size_t _cs=strlen(_mobj)+strlen(a)+256;char*o=malloc(_cs);snprintf(o,_cs,"({sage_rt_array_push(%s,%s);sage_rt_nil();})",_mobj,a);free(_mobj);free(_mn);free(a);return o;}
        ...
        case EXPR_INTERP: {
            /* InterpExpr stores raw template_str; emit as string literal for now */
            const char* tpl = expr->as.interp.template_str ? expr->as.interp.template_str : "";
            char* esc = aot_escape(tpl);
            char* out = malloc(strlen(esc)+32);
            sprintf(out, "sage_rt_string(\"%s\")", esc);
            free(esc); return out;
        }
    ...
    switch (stmt->type) {
        ...
        case STMT_LET: {
            ...
            if (stmt->as.let.initializer) {
                JitTypeTag t=aot_infer_expr(aot,stmt->as.let.initializer);
                if (jit_is_unboxed(t)) {
        ...
        case STMT_EXPRESSION: {
            ...
            if (e && e->type == EXPR_SET && e->as.set.object == NULL) {
                char name[256];
        ...
        case STMT_YIELD:
            ...
            else aot_emit(aot,"return sage_rt_nil(); /* yield */");
            break;
        case STMT_STRUCT: {
        ...
        case STMT_IMPORT: aot_emit(aot,"/* import %s */",stmt->as.import.module_name?stmt->as.import.module_name:"?"); break;
...
static int _yields_are_sequential(Stmt* body) {
    for (Stmt* s = body; s; s = s->next) {
        if (s->type == STMT_WHILE || s->type == STMT_FOR) {
            if (_has_yield(s->type==STMT_WHILE?s->as.while_stmt.body:s->as.for_stmt.body))
                return 0; // yield inside loop = not sequential
        }
        if (s->type == STMT_IF) {
...
static void aot_emit_proc(AotCompiler* aot, Stmt* s) {
    ProcStmt* ps = (s->type==STMT_PROC)?&s->as.proc:&s->as.async_proc;
    char* fname = aot_cname_tok(ps->name);
    ...
    if (_has_yield(ps->body)) {
        if (!_yields_are_sequential(ps->body)) {
            ...
            aot_infer_body(aot, ps->body);
            // Replace yield stmts: yield x → sage_rt_coro_yield(_co, x)
            // We need to compile the body but intercept STMT_YIELD
            // For now: compile body normally (STMT_YIELD already emits return in compile_stmt)
            // We need special yield handling here.
            // Override: compile body manually tracking yields
            for (Stmt* bs = ps->body; bs; bs = bs->next) aot_compile_stmt(aot, bs);
...
char* aot_compile_program(AotCompiler* aot, Stmt* program) {
    ...
    for (Stmt* s = program; s; s = s->next) {
        if (s->type == STMT_PROC || s->type == STMT_ASYNC_PROC) {
            ...
            if (_has_yield(ps->body)) continue;
    ...
    for(Stmt*s=program;s;s=s->next){
        if(s->type==STMT_ENUM){
            ...
            for(int i=0;i<es->variant_count;i++){
                ...
                int has_fields=(es->variant_field_counts&&es->variant_field_counts[i]>0);
                if(!has_fields){
                    // Simple enum: store the tag value in the dict
                    aot_emit(aot,"sage_rt_dict_set(%s,sage_rt_string(\"%s\"),%s_%s());",en,vraw,en,vn);
                } else {
                    // ADT: store constructor function pointer reference — just call it with no args for now
                    aot_emit(aot,"/* ADT variant %s.%s: call %s_%s(...) with args */",en,vraw,en,vn);
                }
                free(vn);
            }
            free(en);
