...
void register_stdlib_modules(ModuleCache* cache) {
    ...
    create_gc_module(cache);
    create_mem_module(cache);
    create_channel_module(cache);
    create_atomic_module(cache);
    create_socket_module(cache);
    create_sandbox_module(cache);
    ...
    // create_graphics_module removed
    create_ml_native_module(cache);
#ifndef SAGE_NO_FFI
    create_ffi_module(cache);
#endif
#ifndef SAGE_NO_PYTHON
    extern Module* create_python_module(ModuleCache* cache);
    create_python_module(cache);
#endif
}
