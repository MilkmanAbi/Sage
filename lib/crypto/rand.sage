...
proc create(seed):
    ...
    var s = seed
    var s0 = splitmix64_next(s)
    s = s0["state"]
    var s1 = splitmix64_next(s)
    s = s1["state"]
    var s2 = splitmix64_next(s)
    s = s2["state"]
    var s3 = splitmix64_next(s)
    state["s0"] = s0["value"]
    state["s1"] = s1["value"]
    state["s2"] = s2["value"]
    state["s3"] = s3["value"]
...
proc splitmix64_next(state):
    var z = state + 11400714819323198485
    z = u64(z)
...
proc next_u64(state):
    ...
    var s2 = state["s2"]
    var s3 = state["s3"]
    var result = u64(rotl64(u64(s1 * 5), 7) * 9)
    let t = u64(s1 << 17)
    s2 = u64(s2 ^ s0)
    s3 = u64(s3 ^ s1)
    s1 = u64(s1 ^ s2)
...
proc next_float(state):
    var r = next_u64(state) & 4503599627370495
...
proc random_bytes(state, count):
    var result = []
    for i in 0..count:
        result.push(next_u32(state) & 255)
    return result
