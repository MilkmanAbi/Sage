...
proc inspect(val):
    ...
    if t == "string":
        result = result + " " + chr(34) + val + chr(34) + " (len=" + str(val.length) + ")"
    if t == "bool":
        if val:
            result = result + " true"
        else:
    ...
    if t == "nil":
        result = result + " nil"
    if t == "array":
        result = result + " [" + str(val.length) + " elements]"
    if t == "dict":
        result = result + " {" + str(val.keys().length) + " keys}"
    return result

@inline
proc dump(label, val):
    println("[DEBUG] " + label + " = ") + inspect(val)
...
proc trace(message):
    if _trace_enabled:
        var indent = ""
        for i in 0.._trace_depth:
            indent = indent + "  "
        println("[TRACE] ") + indent + message
...
proc watch(watcher, name, value):
    ...
    watcher["history"][name].push(value)
    if old != nil and old != value:
        println("[WATCH] " + name + " changed: " + str(old) + " -> ") + str(value)
...
proc time_it(label, fn):
    ...
    println("[TIME] " + label + ": " + str(elapsed * 1000) + "ms")
    return result
