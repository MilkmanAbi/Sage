...
proc parse(parser, argv):
    ...
    while idx < argv.length:
        ...
        if token.length > 2 and token[0] == "-" and token[1] == "-":
            var name = ""
            for j in 0..token.length - 2:
                name = name + token[2 + j]
                ...
                for i in 0..args.length:
                    if not found_opt and args[i]["type"] == 2 and args[i]["long"] == name:
                        if idx + 1 < argv.length:
                            ...
                            result["errors"].push("Missing value for --" + name)
                        found_opt = true
                        consumed = true
                if not found_opt:
                    ...
                    consumed = true
        # Check short flags (-v)
        if not consumed and token.length == 2 and token[0] == "-" and token[1] != "-":
                ...
                if not found_short:
                    ...
                    if args[i]["type"] == 2 and args[i]["short"] == short_ch:
                        if idx + 1 < argv.length:
                            ...
                            result["errors"].push("Missing value for -" + short_ch)
                        found_short = true
                        consumed = true
        if not consumed:
            # Positional argument
...
proc help_text(parser):
    let nl = chr(10)
    var text = "Usage: " + parser["name"] + " [OPTIONS]"
    let args = parser["args"]
    for i in 0..args.length:
        if args[i]["type"] == 3:
            text = text + " <" + args[i]["name"] + ">"
    text = text + nl + nl
    if parser["description"].length > 0:
