...
proc report(suite):
    ...
    println("=== " + suite["name"] + " ===")
    println(str(suite["passed"]) + " passed, " + str(suite["failed"]) + " failed / " + str(total) + " total")
    if suite["failed"] > 0:
        let errors = suite["errors"]
        for i in 0..errors.length:
            println("  FAIL: " + errors[i]["test"] + " - " + errors[i]["message"])
    if suite["failed"] == 0:
...
proc assert_array_equal(actual, expected, message):
    if actual.length != expected.length:
        raise "assert_array_equal failed: " + message + " (length " + str(actual.length) + " != " + str(expected.length) + ")"
...
proc benchmark(name, fn, iterations):
    ...
    println(name + ": " + str(iterations) + " iterations in " + str(elapsed) + "s (" + str(per_iter * 1000) + "ms/iter)")
    let result = {}
    result["name"] = name
