...
typedef struct {
    Expr*  object;
    Token  property;
    // Inline cache: stores the field slot index for instance access
    // If cached_class_id matches the instance's class_id, use cached_slot directly
    unsigned long long cached_class_id;  // class version/id at cache time
    int                cached_slot;      // field index (-1 = not cached)
} GetExpr;
...
// PHASE 6: Interpolated string
// Stores the raw template; interpolation expressions are re-parsed at eval time
typedef struct {
    char* template_str;  // the full raw template (without outer quotes)
    int   template_len;
} InterpExpr;

// PHASE 2: Option/Result expression structs
typedef struct {
    Expr* operand;   // expr! or expr?
} UnwrapExpr;
...
typedef struct {
    Expr* low;       // lo..hi or lo..=hi
    Expr* high;
    int inclusive;   // 1 if ..= (inclusive), 0 if .. (exclusive)
} RangeExpr;

struct Expr {
    enum {
        EXPR_NUMBER,
        EXPR_INT,       // P1: integer literal
        EXPR_STRING,
        EXPR_BOOL,
        EXPR_NIL,
        EXPR_BINARY,
        ...
        EXPR_FORCE_UNWRAP,  // expr!  — panic if nil
        EXPR_PROPAGATE,     // expr?  — early return if nil/Err
        EXPR_NULLCOAL,      // left ?? right
        EXPR_OPTCHAIN,      // expr?.member
        EXPR_RANGE,         // lo..hi  or  lo..=hi
        EXPR_INTERP,        // "Hello, {name}!" — interpolated string
    } type;
    union {
        NumberExpr number;
        IntExpr    int_val;     // P1: EXPR_INT
        StringExpr string;
        BoolExpr boolean;
        BinaryExpr binary;
        ...
        UnwrapExpr   unwrap;        // EXPR_FORCE_UNWRAP, EXPR_PROPAGATE
        NullCoalExpr nullcoal;      // EXPR_NULLCOAL
        OptChainExpr optchain;      // EXPR_OPTCHAIN
        RangeExpr    range;         // EXPR_RANGE
        InterpExpr   interp;        // EXPR_INTERP
    } as;
};
...
typedef struct {
    ...
    Expr* initializer;
    int is_mutable;             // P4: 1 = var/type-first, 0 = let/const
} LetStmt;
...
// Memory/safety annotation kind for @manual, @gc, @trusted, @unsafe, @owned blocks
typedef enum {
    BLOCK_ANNOT_NONE    = 0,
    BLOCK_ANNOT_MANUAL  = 1,   // @manual — explicit alloc/free, no GC overhead
    BLOCK_ANNOT_GC      = 2,   // @gc — force GC-managed even inside manual context
    BLOCK_ANNOT_TRUSTED = 3,   // @trusted — remove runtime safety checks
    BLOCK_ANNOT_UNSAFE  = 4,   // @unsafe — raw pointers, no checks
    BLOCK_ANNOT_OWNED   = 5,   // @owned — single-owner move semantics
} BlockAnnotation;

typedef struct {
    struct Stmt* statements;
    BlockAnnotation annotation;
    char annotation_arg[64];   // optional argument, e.g. @clib("libname")
} AnnotatedBlockStmt;
...
typedef struct {
    Token name;
    Token* params;
    char** param_type_names;  // Type names as strings for ffi dispatch
    int param_count;
    char* return_type_name;   // Return type as string
    char* lib_name;           // The @clib("libname") value
} ExternStmt;
...
// P3: ADT enum with associated data: enum Shape: Circle(r: float), Rect(w: float, h: float)
typedef struct {
    Token name;
    Token* variant_names;
    int variant_count;
    // P3: Per-variant field info (NULL for simple variants)
    Token** variant_fields;      // variant_fields[i] = array of field name tokens (NULL if no fields)
    int* variant_field_counts;   // variant_field_counts[i] = number of fields (0 if no fields)
} EnumStmt;
...
// P3: impl block — attach methods to a type
typedef struct {
    Token target;    // The type name being implemented on
    Stmt* methods;   // Linked list of proc definitions
} ImplStmt;
...
struct Stmt {
    enum {
        ...
        STMT_IMPL,      // P3: impl block
        STMT_COMPTIME,  // Phase 17: Compile-time block
        STMT_MACRO_DEF,  // Phase 17: Macro definition
        STMT_ANNOTATED_BLOCK,  // Phase 1: @manual/@gc/@trusted/@unsafe/@owned blocks
        STMT_EXTERN,        // Phase 4: extern proc decl for @clib bindings
        STMT_SPAWN          // Phase 5: spawn: <block> — runs block in new thread
    } type;
    union {
        PrintStmt print;
        LetStmt let;
        IfStmt if_stmt;
        BlockStmt block;
        AnnotatedBlockStmt annotated_block;
        ...
        ImplStmt impl_stmt;     // P3: impl block
        ComptimeStmt comptime;  // Phase 17: Compile-time block
        MacroDefStmt macro_def; // Phase 17: Macro definition
        Expr* expression;
        ExternStmt extern_proc;  // STMT_EXTERN
        SpawnStmt  spawn_stmt;   // STMT_SPAWN
    } as;
...
Expr* new_int_expr(int64_t value);  // P1
...
Stmt* new_annotated_block_stmt(Stmt* statements, BlockAnnotation annotation, const char* arg);
// Phase 2 expression constructors
Expr* new_force_unwrap_expr(Expr* operand);
Expr* new_propagate_expr(Expr* operand);
Expr* new_nullcoal_expr(Expr* left, Expr* right);
Expr* new_optchain_expr(Expr* object, Token member);
Expr* new_range_expr(Expr* low, Expr* high, int inclusive);
