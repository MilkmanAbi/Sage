...
#include <stdint.h> // int64_t (P1)
#include <math.h>   // fmod (P1 arithmetic helpers)
...
struct ClassValue {
    ...
    int is_struct;     // P2: 1 = value type (copy on assignment), 0 = reference type
};
...
typedef struct {
    ...
    int type_tag;   // P5: 0=raw, 99=Python object
} PointerValue;
...
typedef enum {
    VAL_INT,       // P1: 64-bit signed integer (int64_t)
    VAL_NUMBER,    // IEEE 754 double (float)
    VAL_BOOL,
    VAL_NIL,
    VAL_STRING,
    VAL_FUNCTION,
...
struct Value {
    ValueType type;
    union {
        int64_t  integer;   // P1: VAL_INT — real 64-bit integer
        double number;
        int boolean;
        char* string;
        NativeFn native;
...
#define IS_NUMERIC(v) ((v).type == VAL_INT || (v).type == VAL_NUMBER)
...
// P1: Extract a double from either VAL_INT or VAL_NUMBER
#define NUMERIC_AS_DOUBLE(v) ((v).type == VAL_INT ? (double)(v).as.integer : (v).as.number)
// P1: Extract an int64_t from either VAL_INT or VAL_NUMBER
#define NUMERIC_AS_INT(v) ((v).type == VAL_INT ? (v).as.integer : (int64_t)(v).as.number)
...
#define SAGE_AS_INT(v)    (IS_INT(v) ? (v).as.integer : 0LL)
...
// Constructors
static inline Value val_int(int64_t value) {
    Value v;
    v.type = VAL_INT;
    v.as.integer = value;
    return v;
...
// P1: Arithmetic promotion helpers (shared by interpreter.c and vm.c)
static inline Value sage_add(Value a, Value b) {
    if (IS_INT(a) && IS_INT(b)) return val_int(AS_INT(a) + AS_INT(b));
    return val_number(NUMERIC_AS_DOUBLE(a) + NUMERIC_AS_DOUBLE(b));
}
static inline Value sage_sub(Value a, Value b) {
    if (IS_INT(a) && IS_INT(b)) return val_int(AS_INT(a) - AS_INT(b));
    return val_number(NUMERIC_AS_DOUBLE(a) - NUMERIC_AS_DOUBLE(b));
}
static inline Value sage_mul(Value a, Value b) {
    if (IS_INT(a) && IS_INT(b)) return val_int(AS_INT(a) * AS_INT(b));
    return val_number(NUMERIC_AS_DOUBLE(a) * NUMERIC_AS_DOUBLE(b));
}
static inline Value sage_div(Value a, Value b) {
    if (IS_INT(a) && IS_INT(b)) {
        int64_t bv = AS_INT(b);
        if (bv == 0) return val_int(0);
        return val_int(AS_INT(a) / bv);
    }
    double bd = NUMERIC_AS_DOUBLE(b);
    if (bd == 0.0) return val_number(0.0);
    return val_number(NUMERIC_AS_DOUBLE(a) / bd);
}
static inline Value sage_mod(Value a, Value b) {
    if (IS_INT(a) && IS_INT(b)) {
        int64_t bv = AS_INT(b);
        if (bv == 0) return val_int(0);
        return val_int(AS_INT(a) % bv);
    ...
    double bd = NUMERIC_AS_DOUBLE(b);
    if (bd == 0.0) return val_number(0.0);
    return val_number(fmod(NUMERIC_AS_DOUBLE(a), bd));
}
static inline Value sage_negate(Value a) {
    if (IS_INT(a)) return val_int(-AS_INT(a));
    return val_number(-AS_NUMBER(a));
}
...
Value instance_copy(Value original);  // P2: deep copy for struct value semantics
